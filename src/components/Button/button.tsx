...
export const ButtonGreen_2 = ({ text, to = null }: { text: string; to?: string | null }) => {
  return (
    <Link
      href={`${to ? to : '#'}`}
      className={`bg-neonGreen text-white px-8 py-3 font-medium border-2 border-white transition-all duration-300   hover:bg-white hover:text-neonGreen hover:border-neonGreen `}
    >
      {text}
    </Link>
  )
